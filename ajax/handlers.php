...
// ====== START TAILSCALE PYTHON CONTROLLER (UBUNTU LINUX ONLY) ======
    if ($act === 'tailscale_command') {
        ...
        $py_path = dirname(__DIR__) . DIRECTORY_SEPARATOR . 'tools' . DIRECTORY_SEPARATOR . 'tailscale_control.py'; // moved to tools/
   ...
   if($act==='upload_channel_logo'){
    ...
    if($act==='download_subtitle'){
        ...
        curl_setopt_array($ch2,[
            ...
            CURLOPT_USERAGENT      => OS_UA,
            CURLOPT_IPRESOLVE      => CURL_IPRESOLVE_V4,
            CURLOPT_RESOLVE        => [$dl_host.':443:104.21.75.83', 'api.opensubtitles.com:443:104.21.75.83']
        ]);
        $srt   = curl_exec($ch2);
